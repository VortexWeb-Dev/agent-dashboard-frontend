...
export default function useBitrixAuth() {
  ...
  useEffect(() => {
    ...
    function tryRefresh(refreshToken) {
      ...
      axios
        .post("https://ec2-gicrm.ae/agent-dashboard-backend/?endpoint=bitrixAuth", payload, {
        ...
        .then((res) => {
          ...
          return fetchUser(access_token, domain || import.meta.env.VITE_PORTAL_DOMAIN);
        })
    ...
    function redirectToBitrix() {
      const authUrl = `https://${import.meta.env.VITE_PORTAL_DOMAIN}/oauth/authorize/?client_id=${import.meta.env.VITE_CLIENT_ID}&response_type=code&redirect_uri=${import.meta.env.VITE_REDIRECT_URI}`;
